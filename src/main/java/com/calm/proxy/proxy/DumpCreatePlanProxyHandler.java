...
public class DumpCreatePlanProxyHandler implements ProxyHandler {
    ...
    public void doHandle(ChannelHandlerContext ctx, FullHttpRequest request, HttpHeaders headers) {
        ...
        //创建客户端连接目标机器
        connectToRemote(ctx, URI.create(request.uri()).getHost(), 80, 10000,new HttpObjectAggregator(1024 * 1024 * 1024), new EmptyHandler()).addListener(new AfterConnectionListener(ctx, request, headers));
    }
